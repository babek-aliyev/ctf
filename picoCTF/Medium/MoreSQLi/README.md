...
```sql
SELECT id FROM users WHERE password = 'abc' OR 1=1 --' AND username = 'admin'
```
* `SELECT id FROM users` => this command searches for `id` column inside `users` table and returns us ids of existing users
